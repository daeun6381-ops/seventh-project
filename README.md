...
<head>
    ...
    <style>
        ...
        /* 헤더 고정 해제: sticky 대신 relative/static 사용 */
        header { transition: background 1s ease; position: relative; }
    </style>
</head>
<body id="main-body" class="bg-slate-50 text-slate-800">
    ...
    <header id="main-header" class="bg-white/70 backdrop-blur-md py-8 px-4 text-center border-b border-white/20">
        <div class="max-w-4xl mx-auto relative flex flex-col items-center">
            <button id="btn-logout" class="absolute top-0 right-0 text-[10px] text-slate-400 hover:text-slate-600 underline uppercase">연결 해제</button>
            ...
            <div class="flex flex-col items-center mt-3">
                ...
                <div class="flex items-center gap-2 text-xs text-slate-500 mt-1">
                    ...
                    <span>함께한 지 <span id="days-count" class="font-bold">0</span>일</span>
                </div>
            </div>
            ...
            <div id="partner-mood-status" class="md:absolute md:top-6 md:right-0 mt-4 md:mt-0 text-sm bg-white/80 border border-white/50 px-4 py-2 rounded-full hidden flex items-center gap-2 shadow-sm">
                <span class="text-slate-500 font-medium text-xs">상대방 기분:</span> 
    ...
    <main class="max-w-4xl mx-auto p-6">
            ...
            <div class="space-y-4">
                ...
                <div class="flex flex-wrap justify-between items-center gap-4 pt-2">
                    <div class="flex items-center gap-3 bg-white/50 px-4 py-2 rounded-2xl border border-slate-200">
                        <span class="text-xs text-slate-500 font-medium">지금 내 기분</span>
        ...
        <section>
            ...
            <div id="unlock-msg" class="hidden mb-6 p-4 bg-white/60 backdrop-blur-md border border-amber-200 rounded-2xl text-amber-700 text-sm text-center font-medium shadow-sm">
                🔒 두 사람 모두 오늘 일기를 써야 상대방의 일기를 확인할 수 있어요!
            </div>
    ...
    <script type="module">
        ...
        const createShareCode = async () => {
            ...
            } catch (err) { alert("Code Creation Failed"); }
        };
        ...
        const joinCouple = async () => {
            ...
            try {
                ...
                if (!roomSnap.exists()) return alert('코드를 다시 확인하세요.');
        ...
        function renderDiaries(col, lastWrittenDates) {
            const todayStr = getTodayString();
            const myLastWritten = lastWrittenDates[currentUser.uid];
            const partnerId = Object.keys(lastWrittenDates).find(id => id !== currentUser.uid);
            const partnerLastWritten = partnerId ? lastWrittenDates[partnerId] : null;
            
            // 오늘 두 사람 모두 일기를 썼는지 확인
            const isTodayUnlocked = (myLastWritten === todayStr) && (partnerLastWritten === todayStr);
            
            const unlockMsg = document.getElementById('unlock-msg');
            if (partnerId && !isTodayUnlocked) {
            ...
            onSnapshot(col, (snapshot) => {
                ...
                if (diaries.length === 0) {
                    list.innerHTML = '<div class="col-span-full py-20 text-center text-slate-400 border-2 border-dashed border-white/40 rounded-3xl">첫 번째 일기를 기다리고 있어요.</div>';
                    return;
                }
                ...
                diaries.forEach(data => {
                    ...
                    // 잠금 조건:
                    // 1. 내 일기가 아님
                    // 2. 해당 일기 날짜에 두 사람 모두 일기를 쓰지 않았음
                    const bothWroteOnThatDay = (lastWrittenDates[currentUser.uid] === diaryDate) && (partnerId && lastWrittenDates[partnerId] === diaryDate);
                    const isLocked = !isMyDiary && !bothWroteOnThatDay;
                    ...
                    if (isLocked) {
                        card.innerHTML = `
                            ...
                            <div class="lock-overlay">
                                ...
                                <p class="text-[10px] font-bold text-slate-500 uppercase">Hidden Until Both Write</p>
                            </div>
                        `;
                    } else {
        ...
        const saveDiary = async () => {
            ...
            if (!title || !content) return alert('제목과 내용을 적어주세요.');
